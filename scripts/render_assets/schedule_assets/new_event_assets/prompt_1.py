...
class Prompt_1():
    ...
    def render(self):
        ...
        # render day str
        font_2_gold.render(schedule_obj.day_type.days_data[i],task_obj.page_surf,(int(task_obj.page_surf.get_width() * .05 ),int(task_obj.page_surf.get_height() * .05)))
        ...
        if self.submit_tab.render(self.app.window.display):
            schedule_obj.day_type.days_data_temp[2] = self.app.input.user_text
            # closes current page
            self.page = False
